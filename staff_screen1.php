...
-->
 ...
 <head>
  ...
  margin-bottom: 5px; /* add some spacing between the label and input */
    ...
    <body>      
        <form method ="post">
        Enter The ID or The name of the student's whose info you would like to access <br>
        <br>
...
if (mysqli_num_rows($result) > 0) {
  while($row = mysqli_fetch_assoc($result)) {
      ...
      array_push($ids_from_database,$row["ID"]);
      array_push($names_from_database,$row["Name"]);
...
mysqli_close($mysqli);
        ...
        for($i=0;$i<$max;$i++){
            ...
            if ($StudentID!=NULL and $StudentID==$ids_from_database[$i]){
                ...
                ob_start();
                header("Location:http://3.144.93.168/homepage/staff_screen2.php");
                exit();
